"""Build Milvus Lite index for FastAPI source code — v7min variant.

v7min = text_for_embedding uses signature + docstring ONLY (no source_excerpt),
matching the pre-v6 indexing strategy.

Usage:
    python pe/build_index_v7min.py
...
MILVUS_DB = PE_DIR / "milvus_lite_v7min.db"
...
def _make_chunk(node, file_rel: str, filepath: Path, extra_context: str, parent_name: str | None):
    ...
    # v7min: signature + docstring only (no source_excerpt), matching pre-v6 behavior
...
def main():
    ...
    print("=" * 50)
    print("RepoMind Index Builder v7min")
    print("=" * 50)
