"""RepoMind Benchmark — evaluation pipeline.

Reads questions.jsonl, calls DeepSeek for answers, judges with LLM-as-judge,
outputs results to benchmark/results/baseline.json.
"""
...
def trim_context(snippet: str) -> str:
    ...
    # Keep first 3 non-empty lines (function def + params)
    kept = []
    for line in lines:
        kept.append(line)
        if len(kept) >= 3:
            break
        # Also break if we hit the opening """ (docstring start)
...
def build_answer_prompt(
    ...
    When PE features are enabled:
      - system_prompt replaces the default role description
      - fewshot_examples are inserted before the question
      - CoT guidance is appended to the system prompt
    """
    # System prompt
    if system_prompt:
...
def main():
    parser = argparse.ArgumentParser(description="RepoMind benchmark evaluation")
    ...
    parser.add_argument("--output-name", default=None,
                        help="Custom output file name (default: auto from pe-version)")
    ...
    rag_collection = None
    if args.enable_rag:
        print(f"[RAG] Loading Milvus collection '{args.rag_collection}'...")
        try:
            ...
            print(f"[RAG] Collection loaded. Entities: {rag_collection.num_entities}")
        except Exception as e:
            print(f"[RAG] WARNING: Failed to load RAG: {e}")
            rag_collection = None
    ...
    if args.output_name:
        ...
        output_name = f"pe_v{pe_version}.json"
    else:
        output_name = "baseline.json"
    output_file = RESULTS_DIR / output_name

    print(f"=== RepoMind Benchmark ===")
    ...
    for i, q in enumerate(questions):
        ...
        time.sleep(0.5)  # Rate limit between questions
